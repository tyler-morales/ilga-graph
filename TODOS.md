...
- **main.py split into focused modules (2026-02-18):**
  - **app_state.py** — `AppState` class and global `state`; populated in main lifespan. Removes ~40 lines from main and gives routers/GraphQL a single import for state.
  - **constants.py** — `CATEGORY_COMMITTEES`, `CATEGORY_CHOICES` for advocacy and explore. Removes ~45 lines from main.
  - **startup_banner.py** — `format_startup_table()`, `log_startup_timing()`, `_Colors` for the ETL startup banner and CSV timing log. Removes ~240 lines from main.
...
- **UI consistency and advocacy cleanup (2026-02-19):**
  - Error display: single place — error shown only inside `#results` (index + partial) so full-page and HTMX behave the same.
  - Drawer naming documented in base.html (chrome = phone-overlay/phone-panel; content = drawer-*). Standalone templates (letter_template.html, logs.html) noted as intentionally no base.
  - Drawer JS: `openAdvocacyDrawerFromDrawer(view)` in index.html; no-answer and email partials use it instead of inline long onclick.
...
- **Legacy purge (2026-02-18):**
  ...
  - **Removed test mode and deep link for call/email (2026-02-18):** GET `/advocacy` no longer accepts `?test=1&member_id=&zip=&view=call|email` or deep-link params to auto-open the advocacy drawer. Removed from `main.py`: test_mode, test_zip, test_member_id, test_view, test_members handling and template context. Removed from `index.html`: `#test-mode-open` div and the IIFE that opened the drawer on load for test mode or for `member_id`+`zip`+`view` URL params. ZIP input value now uses `{{ zip or '' }}` only. `/advocacy/test` dev page unchanged. **Superseded by Dev Bar below.**

---

## Current

- **Advocacy page error states (2026-02-19):**
  - **Drawer load:** `openAdvocacyDrawer` now checks `r.ok`; shows "Legislator not found." for 404 and "Something went wrong. Try again." for other non-2xx; `.catch()` still shows "Could not load. Try again." for network errors.
  - **"I sent" button:** Only shows "Sign in to save this outreach" when server returns an auth-related error; shows "Couldn't save. Try again." for network or other failures (resets to "I sent" after 3s).
  - **Call wrap-up fetch:** On non-2xx or network failure, removes loading state and prepends a `.drawer-error-banner` in the drawer: "Couldn't load follow-up. Try again."
  - **Search empty state:** When ZIP is valid but no cards (your_legislators, broker, ally, super_ally all empty), `_results_partial.html` shows a `.results-empty` message: "No targets found for this ZIP. Try another Illinois ZIP or policy area."
  - **Drawer 404:** `GET /advocacy/drawer` returns 404 with `{"detail": "Legislator not found."}` when `member_id` is provided but the member is not in state; frontend already shows "Legislator not found." for 404.
  - **Files changed:** `index.html` (drawer fetch + sent btn + wrapup error), `base.html` (`.drawer-error-banner`, `.results-empty`), `_results_partial.html` (empty-state branch), `main.py` (advocacy_drawer 404).
...
- **Guided Email Flow — "Mad Libs" Compose Experience (2026-02-18):**
  ...
  - **Mad Libs body:** Email body `<textarea>` replaced with inline `<input class="madlib-blank">` elements parsed from `[CALLER_NAME]`, `[CALLER_CITY]`, `[CALLER_ZIP]`, `[CALLER_PHONE]`, `[CALLER_EMAIL]` placeholders. Filling one instance auto-fills duplicates. ZIP and email pre-filled from drawer context and auth state.
  - **Subject verify button:** New "Looks good" checkmark button on the right side of the subject row. Click confirms the subject and advances the guide to body fill-in.
  - **Guide step dots:** Six dots in the toolbar header track progress with active/done states.
  - **PDF download nudge:** Guide pulses the attachment bar after body completion; click on the PDF link advances the guide to "Open in email client."
  - **Sent confirmation:** "I sent" button turns green with "Sent!" text and all guide dots go green on successful outreach recording.
  - **Validation:** Clicking "Open in email client" with unfilled blanks bounces the guide back to the first empty blank with a red error pulse. Blanks validate on Tab progression.
  - **Auth integration:** `goVerified()` hook in `initDrawerFromAuth` notifies the guide to advance past sign-in step and pre-fill `[CALLER_EMAIL]` blanks.
  - **Filled blanks → read-only text:** Once a blank has a value and loses focus, it is shown as normal text with a green underline (`.madlib-filled`). Clicking it makes it editable again. Phone number is optional (validation skips `CALLER_PHONE` / `YOUR PHONE`).
  - **Next-step ask choice:** The line "Thank you for your time — what's the best next step from here, and is there a staff member I should coordinate with on transportation/vehicle policy?" is now a placeholder `[NEXT_STEP_ASK]`. User selects one of: Full (full sentence), Short ("What's the best next step from here?"), or Omit. Backend: `build_email_body_full` in `advocacy_helpers.py` uses `[NEXT_STEP_ASK]`; frontend renders a dropdown and injects the chosen text into the composed body.
  - **Editable To field (2026-02-18):** Email drawer "To" is now an editable `#gmail-to-input` (type=email) pre-filled with `recipient_email` when present; placeholder "Recipient email address" when empty. Mailto and copy use `getRecipient()` from the input so users can paste or correct the address.
  - **Files changed:** `_advocacy_drawer_email.html` (template restructure), `base.html` (CSS for guide-pulse, madlib-blank, madlib-filled, madlib-next-step-wrap, subject-verify-btn, sent-done), `index.html` (JS: `renderMadLibsBody`, `getComposedBody`, `EmailGuide`, OPTIONAL_BLANKS, NEXT_STEP_ASK_OPTIONS, getRecipient), `advocacy_helpers.py` (closing uses `[NEXT_STEP_ASK]`).

- **Outreach scripts — dynamic templates (2026-02-18):**
  - **Phone script (answered):** OPEN (name, constituent, “May I leave a quick message for [title] [last] about kei vehicles?”), THE PROBLEM (SOS branding “Not Eligible for Registration,” statute gap), THE LEGAL “WHY” (625 ILCS 5/3-401(c-1), 25+ years lawfully imported), THE ASK (narrow bill: 25+ year kei, limited on-road, no interstates, equipment/insurance), NEXT STEP (best email for one-pager + staffer name), IF THEY PUSH BACK (three short responses: not highway/federal, need more info, can’t commit). Template: `_advocacy_drawer_call.html`.
  - **Voicemail / no answer:** Single bubble: identify yourself, SOS “Not Eligible for Registration” + statute, ask for support for narrow bill (25+ year, limited on-road, restrictions), callback number + request best email for one-pager. Same template.
  - **Email template:** Subject options: default “Constituent request: clarify Illinois registration for kei vehicles (25+ years old)”; alternate “Please support a narrow ‘kei vehicle’ registration fix ([ZIP] constituent)”. Body: Hi [staff], My name is [CALLER_NAME], constituent; following up on call (or reaching out); What’s happening (SOS off-highway/titles); Why (3-401(c-1)); My ask (bullets: Kei category 25+, limited on-road, equipment/insurance/docs); two-page brief note; next step + staff contact ask; Sincerely + [CALLER_NAME], [CALLER_CITY], IL [CALLER_ZIP], [CALLER_PHONE]. Greeting: if contact name from call wrap-up → "Good [morning/afternoon/evening], [Name],"; else "Dear Office of Rep./Sen. [Last],". Helpers: `build_email_subject_line`, `build_email_body_full`, `_time_of_day_greeting` in `advocacy_helpers.py`.
  - **Placeholders:** [CALLER_NAME], [CALLER_CITY], [CALLER_ZIP], [CALLER_PHONE] (email body); drawer intro/voicemail use same + ZIP from search when constituent checked.
  - **Drawer:** GET `/advocacy/drawer?view=email` accepts `target_type`; POST `/advocacy/call/<id>/wrapup` accepts `target_type` and `call_date` so follow-up email gets correct ASK and "Thank you for taking my call [date]".
  - **Frontend:** Email button on cards passes `data-target-type` (POWER_BROKER for broker/super-ally, NON_COMMITTEE else); drawer fetch includes `target_type` for both call and email views; wrapup form submits `target_type` and `call_date` (today) for after-call email.

- **Deployment readiness — advocacy + prod (2026-02-18):**
  - Root `/` now redirects to `/advocacy` so the site has a clear entry point. `/` is API-key exempt.
  - `/intelligence` (and sub-pages) added to API-key exempt list so the Intelligence dashboard is reachable in prod when `ILGA_API_KEY` is set; only `/graphql` and programmatic API routes then require the key.
  - `docs/reference/deployment.md` — new "Advocacy systems (ready for deployment)" section summarizing routes, auth/outreach exemption, SSR exemptions, DB init, and optional letter template.

- **Outreach heat pill — community advocacy indicator (2026-02-18):**
  - **What:** Each member card now shows a fire-emoji pill (🔥 N) in the top-right corner when at least one unique advocate has called or emailed that legislator. Clicking the pill expands it horizontally to read "🔥 N unique outreach advocates".
  - **Backend:** `advocacy_search` in `main.py` queries `COUNT(DISTINCT user_id)` grouped by `member_id` for all result cards, filtering to call/email events. Passes `outreach_heat: dict[str, int]` to the template context.
  - **Template:** `member_card` macro in `_results_partial.html` accepts the new `outreach_heat` dict. When `heat_count > 0`, renders a `.heat-score-pill` span with `onclick` toggle for `data-expanded` attribute.
  - **CSS:** `.heat-score-pill` in `base.html` — warm orange styling, `cursor: pointer`, expandable `.heat-score-pill-text` child using `max-width` / `opacity` transition on `[data-expanded]`.
  - **Two-DB setup:** Dev profile → `data/ilga_dev.db` (sandbox); prod → `data/ilga.db` (live). Profile-based default in `config.py`; override with `ILGA_DB_PATH`. App and seed script both use the same profile from env.
  - **Seed data:** Real backlog (user table) always seeded for `moratyle@gmail.com`. When `ILGA_PROFILE=dev`, `seed_outreach.py` also seeds mock community advocates (advocate1–5@example.com) so heat pills show varied counts in dev; prod seed is real data only.

- **Unified bill scrape pipeline (2026-02-18):**
  - **Problem:** Scraping a bill required up to three separate passes (metadata, votes+slips, fulltext), with the BillStatus page fetched twice per bill. Each phase had its own progress file, logging format, and checkpointing. `bills.json` grew to ~150MB with inline full text, making checkpoint saves slow.
  - **Solution:** Consolidated all bill data fetching into a single per-bill function (`scrape_bill_complete()`) that fetches the BillStatus page **once** and reuses the HTML for metadata parsing + vote tab URL extraction.
  - **Changes:**
    - **Externalized full text** — `Bill.full_text` is no longer serialized into `bills.json`. Instead, `bills.json` stores `has_full_text: true/false` and individual text files live at `cache/fulltext/<bill_number>.txt`. At load time, `_bill_from_dict()` joins full text from disk. `save_fulltext()` writes individual files.
    - **`scrape_votes_from_html()`** — New function in `votes.py` that accepts already-fetched BillStatus HTML instead of re-fetching it. `scrape_bill_votes()` is now a thin wrapper.
    - **`scrape_bill_complete()`** — New function in `bills.py` that does metadata + votes + slips + optional fulltext in one task. Uses `_scrape_bill_status_with_html()` (refactored from `scrape_bill_status`) to get both the Bill object and raw HTML in a single fetch. Stalled-bill heuristic skips sub-scrapes for intro-only bills.
    - **Unified `incremental_bill_scrape()`** — Refactored to call `scrape_bill_complete()` per bill with checkpointing every 50 bills, consistent progress logging (`[N/total] BILL — Xm Ys elapsed, ~Wm Zs remaining`), and phase summaries.
    - **Performance** — Default `max_workers` increased from 3 to 10; connection pool sized to match; `--fast` delay reduced from 0.15s to 0.1s.
    - **Pipeline orchestration** — `scripts/scrape.py` no longer spawns a Phase 3 subprocess for `scrape_votes.py`. Votes and slips are part of the unified per-bill scrape. Added `--fulltext` flag. Total scrape time printed at end.
    - **Makefile** — `make scrape FULLTEXT=1` includes full text; `make scrape-fulltext` kept for standalone backfill.
    - **Log utility** — `src/ilga_graph/scrapers/_log.py` with `fmt_duration()`, `log_progress()`, `log_phase()`.
    - **Scrape terminal output (2026-02-18):** One consistent line per bill when full data is scraped. Sub-scraper progress (witness slips "Found N versions", vote PDF download/parse) moved to DEBUG so the terminal shows only `[N/total] BILL_NUMBER — Xm Ys elapsed, ~Wm Zs remaining` per completed bill in both `scrape_all_bills` and `incremental_bill_scrape`. Warnings/errors remain at INFO.
  - **Files changed:**
    - `src/ilga_graph/scrapers/bills.py`: `_bill_to_dict`, `_bill_from_dict`, `save_bill_cache`, `save_fulltext`, `_scrape_bill_status_with_html`, `scrape_bill_complete`, `_is_stalled_bill`, refactored `incremental_bill_scrape`
    - `src/ilga_graph/scrapers/votes.py`: `scrape_votes_from_html` extracted
    - `src/ilga_graph/scrapers/_log.py`: New log utility module
    - `src/ilga_graph/etl.py`: `load_or_scrape_data` passes `include_votes`, `include_slips`, `include_fulltext`, `max_workers` through
    - `scripts/scrape.py`: Removed Phase 3 subprocess; added `--fulltext`; consistent phase/total timing
    - `scripts/scrape_votes.py`: Kept as standalone; updated `--fast` delay to 0.1s
    - `scripts/scrape_fulltext.py`: Updated `--fast` delay to 0.1s
    - `Makefile`: Added `FULLTEXT=1` flag; updated help comments

- **Auth + outreach DB — passwordless email-code login + outreach tracking (2026-02-18):**
  - **Problem:** No user accounts and no persistence of advocacy outreach (calls, emails). Heat scores and outreach counts were mock data only.
  - **Solution:** Lightweight SQLite DB (`data/ilga.db`) with async SQLAlchemy + aiosqlite. Passwordless email-code auth (6-digit OTP). Outreach event recording per authenticated user.
  - **Auth flow:** "From" field added to the Gmail-style email drawer. When not signed in, user types their email → clicks **Send code** → receives a 6-digit code (logged to console in dev, sent via SMTP in prod) → enters code → **Confirm** → verified. Session cookie (`ilga_session`) set via `itsdangerous` signed token. Once verified, "From" shows a green verified chip; email is prefilled on all subsequent email drawers.
  - **Hero inline sign-in (2026-02-18):** Hero meta reorganized into one line: "Data from ilga.gov · N legislators tracked · ML-scored influence rankings · Sign in" (or "Signed in as … · Sign out"). No long explanatory sentence; clicking **Sign in** reveals the inline form (email → Send code → 6-digit code → Confirm). Uses same `/auth/request-code` and `/auth/verify-code`. Styles: `.hero-meta`, `.hero-meta-line`, `.hero-signin-inline` in `base.html`; JS in `index.html` `initHeroInlineSignin()`.
  - **Unified sign-in animation and copy (2026-02-19):** Hero and email drawer sign-in use the same flow and UI copy for consistency. Both show **Sending…** / **Verifying…** with disabled state (opacity 0.5, cursor wait); button labels **Send code** (step 1) and **Confirm** (step 2); same error messages ("Invalid or expired code. Try again or resend.", "Network error — try again"). Hero `.hero-signin-btn:disabled` and `.hero-signin-resend:disabled` match drawer `.gmail-from-verify-btn:disabled` styling.
  - **Email drawer sign-in UX (2026-02-19):** In the email drawer From section: **Enter** in the email field sends the code when the address is valid (contains `@`). **Send code** is disabled until a valid email is entered (with `@`); it becomes enabled and blue when valid. **Confirm** (6-digit code step) is disabled until 6 digits are entered, then enabled and blue. All primary confirm actions (Send code, Confirm) use blue (#1a73e8) in both the email drawer and hero sign-in for consistency. Hero email field also supports Enter to send code.
  - **DB models:** `User` (id, email, created_at, last_login_at), `AuthCode` (email, code_hash, expires_at, used), `OutreachEvent` (user_id, user_email, member_id, kind, zip_code, outcome, notes, **contact_name**, **support_score** 1–5, **constituent**, created_at). Outreach stores **time/date** (`created_at`), **person who picked up** (`contact_name`), **interest level** (`support_score`: 1=opposed … 5=champion), **constituent** (was advocate a constituent?), and freeform **notes**. Migrations in `init_db()` add new columns for existing DBs.
  - **Call drawer:** After "Nice work! 👋" the wrap-up form includes **How interested did they seem?** with iMessage-style pills: Opposed, Skeptical, Neutral, Interested, Champion (stored as support_score 1–5). `recordOutreach()` sends `support_score` to `POST /outreach/record`.
  - **Interest poll pill colors (2026-02-18):** Bright semantic palette — Opposed red (#FFC4D4/#E63946), Skeptical orange (#FFD9A8/#F97316), Neutral gray (#E8E8ED/#6B7280), Interested green (#B8F0C8/#22C55E), Champion gold (#FFF4B8/#EAB308). Bar fill: hard stop at barPct% (no fade); base.html + index.html `PILL_BAR_COLORS`. **Bar chart fixes (2026-02-18):** Poll result bar text uses a darker shade of each pill color (not white) for readability; 0% bars get class `.poll-bar-zero` with light semantic tint background and opacity 0.78 so they remain visible but clearly empty. **Default pill colors (2026-02-18):** Pre-vote pills use full opacity and the same semantic color scheme as results: rgba tint backgrounds (e.g. rgba(230,57,70,0.28) for Opposed) and dark accent text (C1121F, EA580C, etc.); dot opacity raised from 0.5 to 0.9 so the circle is clearly visible.
  - **Seed script:** `scripts/seed_outreach.py` prefills `data/ilga.db` with past outreach for `moratyle@gmail.com` from a fixed table; rep names are resolved to `member_id` via cache/mocks `members.json`. Run: `make seed-outreach` or `python scripts/seed_outreach.py`.
  - **DB + outreach tests (2026-02-18):** `tests/test_db.py` — init_db creates tables, idempotent migrations, outreach_events schema (contact_name, support_score, constituent), session persistence. `tests/test_auth_outreach.py` — auth (request-code, verify-code, /me, logout), full verify roundtrip with known code, outreach record (401 unauthenticated, 400 invalid kind, success with event_id, support_score/constituent stored), stats (empty and aggregated), my-history (401, ordering, fields), parsing helpers (_parse_support_score, _parse_constituent). Uses temp DB; see `docs/development/db-and-outreach.md` for scan and coverage. Run: `make test` or `pytest tests/test_db.py tests/test_auth_outreach.py -v`.
  - **Outreach smoke test (2026-02-19):** `scripts/smoke_test_outreach.py` — terminal-only automated flow: sign in (pre-seeded code), record one call and one email, assert data saved and that public `GET /outreach/stats/{member_id}` (no auth) shows counts so a visitor would see the outreach; assert my-history returns both events. No server or Brevo required. Run: `make smoke-outreach` or `PYTHONPATH=src python scripts/smoke_test_outreach.py`. Documented in `docs/development/db-and-outreach.md`.
  - **Endpoints:**
    - `POST /auth/request-code` — send a 6-digit code to the given email
    - `POST /auth/verify-code` — verify code, create/get user, set session cookie
    - `POST /auth/logout` — clear session cookie
    - `GET /auth/me` — return current user email or 401
    - `POST /outreach/record` — record a call/email/no_answer event (requires auth)
    - `GET /outreach/stats/{member_id}` — public per-member outreach counts
    - `GET /outreach/my-history` — authenticated user's outreach history
  - **Files created:**
    - `src/ilga_graph/db.py` — async SQLite engine, session factory, `init_db()`, `get_db` dependency
    - `src/ilga_graph/db_models.py` — `User`, `AuthCode`, `OutreachEvent` SQLAlchemy models
    - `src/ilga_graph/dependencies.py` — `get_current_user_optional`, `require_user`, session token helpers
    - `src/ilga_graph/routers/auth.py` — auth router (request-code, verify-code, logout, me)
    - `src/ilga_graph/routers/outreach.py` — outreach recording + stats router
  - **Files changed:**
    - `pyproject.toml` — added `sqlalchemy[asyncio]`, `aiosqlite`, `itsdangerous`, `aiosmtplib`
    - `src/ilga_graph/config.py` — added `AUTH_SECRET`, `AUTH_COOKIE_*`, `SMTP_*` settings
    - `src/ilga_graph/main.py` — `init_db()` in lifespan, auth + outreach routers mounted, `/auth` + `/outreach` exempt from API key middleware
    - `src/ilga_graph/templates/_advocacy_drawer_email.html` — "From" row with email input, verify button, code entry, verified chip, sign-out
    - `src/ilga_graph/templates/index.html` — `initDrawerFromAuth()` JS for auth flow in drawer, `recordOutreach()` on wrapup
    - `src/ilga_graph/templates/base.html` — CSS for `.gmail-from-*`, `.gmail-from-code-*` auth UI
    - `.env.example` — added `ILGA_DB_PATH`, `ILGA_AUTH_SECRET`, `ILGA_SMTP_*` vars
    - `.gitignore` — added `data/` directory
  - **Dev testing:** In dev mode (no SMTP configured), the 6-digit code is logged to the console in a box. No real email service needed for local testing.
  - **Gamified outreach done state (2026-02-18):** When the signed-in user has already called or emailed a legislator, the Call/Email buttons on that card show **Called!** / **Emailed!** with a gold metallic style (gradient, subtle shine animation) and are non-clickable. Encourages completion and makes progress visible. Implemented by: (1) `advocacy_search` now takes `get_db` and `get_current_user_optional`, queries `OutreachEvent` for result member IDs, and passes `user_called_member_ids` / `user_emailed_member_ids` to the results partial; (2) `_results_partial.html` `member_card` macro accepts these sets and renders a `<span class="outreach-done">` instead of a button when done; (3) `base.html` CSS for `.outreach-btn.outreach-done` (gold gradient, box-shadow, optional shine animation with `prefers-reduced-motion` respected); (4) `recordOutreach()` in `index.html` returns the fetch promise and on success calls `setOutreachDone(memberId, kind)` to replace the button with the gold span in the DOM so the UI updates without a full re-search.
  - **Interest-level poll with bar-chart pills (2026-02-18):** "How interested did they seem?" is now its own **Quick poll** chat bubble. Tapping a level (Opposed → Champion) fetches aggregated results from other callers for that member. After voting, each pill transforms into a **horizontal bar-chart bar**: the background fills proportionally with the pill's semantic color (alpha intensity scales with vote share), and count + percentage are shown right-aligned inside each pill. The user's selected pill gets a 2px border accent. Zero-results: "You're the first to report!" message. Summary line shows total advocate count below the pills. Backend: `GET /outreach/interest-poll/{member_id}` returns `total_responses` and `by_score` (1–5). **Colored pills:** Opposed=red, Skeptical=orange, Neutral=gray, Interested=green, Champion=gold. JS `renderInterestPollResults` adds `.poll-voted` to every pill, sets inline `linear-gradient` background proportional to vote share, and populates `.drawer-interest-pill-stats` with count + pct. CSS: `.poll-voted` disables pointer-events and hides the dot; `.poll-voted[aria-pressed="true"]` keeps semantic text color with a solid border; score-specific `:not(.poll-voted)` selectors preserve pre-vote interactive styling.
  - **Dev: Call wrap-up + poll mock (2026-02-18):** On `/advocacy/test`, use **Call wrap-up + poll (mock)** (or quick link **Poll (mock): Member name**) to open the advocacy page with the call drawer already in wrap-up state: "Nice work!" bubble, poll bubble visible with mock data (e.g. 1 Opposed, 2 Skeptical, 2 Neutral, 5 Interested, 3 Champion → bar-chart pills), and draft follow-up sticky note. URL: `/advocacy?dev&zip=60601&member_id=<id>&view=call_wrapup`. JS: `showWrapupWithMockPoll(container)` plus `openAdvocacyDrawer('call_wrapup', ...)` fetches call drawer then runs mock flow.
  - **Draft follow-up button stagger (2026-02-18):** In the call wrap-up, the "Draft follow-up email" button now appears 500ms after the note block (sticky note textarea). CTA block uses `.drawer-draft-followup-cta-staggered` (opacity 0) and `.drawer-draft-followup-cta-visible` (opacity 1, 0.35s transition). JS clears visible when hiding the draft bubble so the stagger runs again on next wrap-up.

- **Cursor subagents (2026-02-18):**
  - **Backend** — `.cursor/agents/backend.md`: Python, FastAPI, GraphQL, ETL, ML. Use for route/resolver implementation and refactors; enforces state-from-context, batch loaders, Ruff config.
  - **Tests** — `.cursor/agents/tests.md`: Adds and updates tests. Uses pytest and httpx in `tests/`; keeps `conftest.py` fixtures and `make test` working. Use proactively when adding features or when tests need to be written or updated.

- **Dev Bar — global dev testing toolbar (2026-02-18):**
  - **Problem:** The old `/advocacy/test` page generated `?test=1` deep links, but main `/advocacy` route no longer accepted those params (removed in legacy purge). Testing required clicking through the full UI. No dev testing support for intelligence or explore pages.
  - **Solution:** Global floating dev toolbar activated by appending `?dev` to any page URL. Persists across page navigations via `sessionStorage`. Close with the X button, `?dev=off`, or `Ctrl+Shift+D` keyboard shortcut. Only rendered when `DEV_MODE` is true (never in prod).
  - **Features:**
    - Sticky dark bar at viewport bottom with nav links (Advocacy, Intelligence, Explore, GraphQL, Logs).
    - **Advocacy panel** (on `/advocacy*`): ZIP input, member dropdown (loaded from `/api/dev/members`), "Open Call" / "Open Email" buttons that directly call `openAdvocacyDrawer()`, "Search ZIP" button that triggers form submit. Deep-link auto-open: `/advocacy?dev&member_id=X&zip=60601&view=call` auto-opens the drawer on page load.
    - **Intelligence panel** (on `/intelligence*`): Chip links to all sub-pages (raw, predictions, coalitions, anomalies, influence, recruitment, committees, accuracy, witness slips), member ID and bill ID inputs with "Go" buttons for quick navigation to `/intelligence/member/{id}` and `/intelligence/bill/{id}`.
    - **Keyboard shortcut:** `Ctrl+Shift+D` toggles the dev bar on/off from any page.
  - **Files changed:**
    - `base.html`: Dev bar HTML, CSS, JS at bottom of `<body>`, gated by `{% if dev_available %}`. URL param detection, `sessionStorage` persistence, context-aware panels.
    - `main.py`: `templates.env.globals["dev_available"] = DEV_MODE` to inject flag into all templates. `GET /api/dev/members` endpoint returns first 20 members as JSON. `GET /advocacy` now accepts optional `zip`, `member_id`, `view` query params. `/api/dev` exempt from API key middleware.
    - `advocacy_test.html`: Updated to use `?dev` param system instead of `?test=1`. Added notice about the new dev bar. Added links to intelligence/explore pages with `?dev`.
  - **Config guard:** Dev bar markup is only rendered when `DEV_MODE` is true (Jinja2 global `dev_available`). Production deployments (`ILGA_PROFILE=prod`) never see the dev bar HTML, CSS, or JS.

- **Separate dev and prod data directories (2026-02-18):**
  - **Problem:** `make dev-seed` destructively overwrote `cache/` with 20-member mock data, wiping full scraped data needed by `make serve`. No isolation between dev and prod.
  - **Solution:** Dev uses `cache/dev/`, prod uses `cache/`. No copying, no overwriting, no ambiguity.
    - `cache/` is **sacred** — only `make scrape` writes to it. `make dev` never reads from or writes to it.
    - `cache/dev/` is the dev cache. If empty, falls back to `mocks/dev/` automatically. Disposable.
    - `make dev-seed` **removed**. No longer needed because dev always reads from `cache/dev/` or falls back to `mocks/dev/`.
    - `make dev-reset` **added** — clears `cache/dev/` so next `make dev` uses seed data.
  - **Files changed:**
    - `config.py`: `CACHE_DIR` now resolves to `cache/dev/` when `ILGA_PROFILE=dev`, `cache/` when prod.
    - `Makefile`: Removed `dev-seed`, added `dev-reset`. All scrape targets (`scrape`, `scrape-members`, `scrape-fulltext`, `refresh-photos`) explicitly set `ILGA_PROFILE=prod` so they always write to `cache/`.
    - `base.html` dev bar JS: Extended `sessionStorage` persistence to include last member_id, view (call/email), and drawer open/closed state. On reload, drawer auto-reopens where you left off. MutationObserver clears drawer state when closed.
    - `docs/getting-started.md`: Documented two-directory setup, removed `dev-seed` references.
    - `docs/reference/cli-make.md`: Replaced `dev-seed` with `dev-reset`, updated target descriptions.

- **Dev mock data — better zero-cache experience (2026-02-18):**
  - **Committees:** `mocks/dev/committees.json` — Added `roster` and `bill_numbers` to SAGR, SCOA, SENE, SEXC, SEXC-AISM so committee stats and advocacy committee features work when loading from seed. Rosters use mock member IDs (3291, 3329, 3317, 3312, 3337); bill_numbers reference mock bill numbers (SB3974, SB3819, etc.).
  - **Bills:** `mocks/dev/bills.json` — Added `action_history`, `vote_events`, and `witness_slips` (empty) to four bills (167471, 167472, 167225, 167334). Vote events use mock member names (Bill Cunningham, Neil Anderson, etc.) so vote normalization and influence (pivotality) run in dev.
  - **Member–bill consistency:** Verified all `sponsored_bill_ids` / `co_sponsor_bill_ids` in mock members reference existing `leg_id`s in mock bills.

- **Member card: no-email UX (2026-02-18):**
  - When a member has no email on file, the Email button stays **clickable** (drawer handles missing email with banners + paste-here To field). Added subtle UX: (1) **Hint text** under the Email button (desktop and mobile), right-aligned: "No email on file — call to get one, or check their website." (2) **Call and Email buttons** same height and width (flex equal share + min-height on `.outreach-btn`, `.outreach-email-wrap .outreach-email` width 100%). (3) **Visual hierarchy**: Call gets a light ring (primary); Email uses muted gray (secondary) so Call is the suggested path without blocking Email. Template: `_results_partial.html` (`.outreach-row-no-email`, `.outreach-email-wrap` wrapping Email button + `.outreach-row-hint`). CSS: `base.html` (`.outreach-email-wrap` column flex, centered; `.outreach-row-no-email .outreach-phone` ring, `.outreach-row-no-email .outreach-email` gray; `.outreach-row-hint` small muted text, `text-align: right`).

- **Favicon (2026-02-18):**
  - Favicon at `GET /favicon.ico` (SVG: truck emoji 🚚 in `static/favicon.svg`). `base.html` has `<link rel="icon" href="/favicon.ico" type="image/svg+xml">`; `/favicon.ico` exempt from API-key middleware.

- **Member cards: badges under name on desktop only (2026-02-18):**
  - **Desktop:** Badges (e.g. YOUR REPRESENTATIVE, LEADERSHIP) now sit directly under the name/district line in the card header, using CSS Grid: `.card-header` is a 2-column grid with photo in column 1 spanning both rows and name + badge row in column 2. Mobile unchanged: header stays flex wrap and badge row uses `flex-basis: 100%` so it remains full width below photo + name. Templates: `_results_partial.html` (badge row is now a direct child of `.card-header`). CSS: `base.html` (grid for desktop, mobile overrides in `@media (max-width: 768px)`).
  - **Card header top whitespace removed (2026-02-18):** `.card-header` used `grid-template-rows: 1fr auto`, which made the first row grow and (with `align-items: end`) pushed content down, leaving bulky empty space above the header. Changed to `grid-template-rows: auto auto` so both rows size to content; CSS in `base.html`.

- **Call drawer: email/name input width (2026-02-18):**
  - Call drawer `.phone-panel` width 520px (`min(520px, 92vw)`). Email/name input wrappers: **75% width, right-aligned** (`.drawer-email-capture-wrap`, `.drawer-name-capture-wrap`: `width: 75%; max-width: 75%; margin-left: auto` in `base.html`).

- **Call drawer: unified lighter gray for conversation flow (2026-02-18):**
  - All gray backgrounds in the call script flow use one lighter gray: `#ebebf0`. Applied to `.chat-them` (staffer bubbles), `.drawer-wrapup-inline-form-bubble`, and `.drawer-email-capture` / `.drawer-name-capture` (default and `:focus`).

- **Email drawer: click-to-copy on member To chip (2026-02-18):**
  - The To chip (name + email) in the Gmail-style email drawer is now clickable: copies the member email to clipboard and shows green "Copied!" feedback for 1.4s. Template: `.gmail-to-chip-copy` with `data-email`, `title="Click to copy email"`. JS: `initDrawerEmailActions` adds click handler; CSS: `.gmail-to-chip-copy` cursor/hover and `.gmail-to-chip-copied` green state in `base.html`.

- **Advocacy drawer width (2026-02-18):**
  - **Call script drawer** uses normal drawer width: `min(420px, 92vw)`. **Email drawer only** uses 50% viewport width (50vw). Implemented via default `.phone-panel` width + `.phone-panel--email` modifier; `openAdvocacyDrawer()` toggles `phone-panel--email` when `view === 'email'`. Tablet breakpoint still forces both to full width.

- **Gmail-style email drawer redesign (2026-02-18):**
  - **Problem:** Clicking "Draft follow-up email" in the call wrap-up navigated to a blank page (the POST endpoint returned an HTML partial with no base template). The email template was also plain and unstyled compared to the fun iMessage-style call script.
  - **Root cause fix — `htmx.process()` for fetch-loaded content:** The drawer HTML was loaded via vanilla `fetch()` and set via `innerHTML`, but HTMX doesn't automatically process dynamically inserted content (only its own swaps). Added `htmx.process(body)` after `body.innerHTML = html` in `openAdvocacyDrawer()` so HTMX discovers `hx-post` attributes on the wrapup form and intercepts the submit instead of falling back to the form's `action` attribute (which caused full-page navigation).
  - **Gmail-compose UI for email drawer:** Completely redesigned `_advocacy_drawer_email.html` to mimic a Gmail compose window: dark toolbar header ("New Message"), From/To/Subject field rows with Gmail-style field labels, To recipient chip (avatar initial + name + email), editable From field (pre-filled with placeholder for user's name/email), compose body area with pre-wrap text, attachment bar (letter-template.pdf styled as Gmail attachment chip), and a bottom action bar with "Open in email client" (blue pill button, mailto) and copy buttons (Subject / Body / All as rounded Gmail-style chips with copy icon + green "Copied!" confirmation state).
  - **Sender pre-fill:** From field is an editable input pre-filled with `[Your Name] <your.email@example.com>` so users can fill in their own details.
  - **Copy & send actions:** Bottom bar has primary "Open in email client" button (sends mailto with recipient, subject, body) and secondary copy pills for individual fields. Copy buttons show green "Copied!" state with `.gmail-copied` class transition.
  - **"I sent" + simplified copy (2026-02-18):** Next to "Open in email client" added **"I sent"** button (id `gmail-sent-email-btn`) so advocates can record that they actually sent the email; when signed in this POSTs to `/outreach/record` (kind=email), shows "Sent!" and the card updates to gold "Emailed!" badge. Copy Subject/Body/All pills removed from the action bar to save space; replaced with a single **"Copy subject & body"** text link in a thin row above the action bar (`.gmail-copy-row`, `.gmail-copy-link`). JS: `initDrawerEmailActions` uses restore label from button text so the copy link reverts to "Copy subject & body" after "Copied!"; "I sent" success feedback only when `data.ok` (signed-in). **Sign-in nudge when not recorded (2026-02-19):** If outreach was not saved (401 or missing `data.ok`), the button no longer shows "Recorded!"; it shows "Sign in to save this outreach", applies `.sent-signin-nudge` (amber styling), and the guide jumps to the sign_in step (From/verify row) via `guide.setStep(0)`; after 3s the label reverts to "I sent". CSS: `.gmail-sent-btn`, `.gmail-sent-btn.sent-signin-nudge`, `.gmail-copy-row`, `.gmail-copy-link` in `base.html`.
  - **Context banners:** Gmail-style banners for different scenarios: warning banners (call-only, no public email, copy-only mode) with amber background, tip banner (call nudge) with green background. Each has an emoji icon and descriptive text.
  - **Responsive:** On mobile (≤480px), compose card goes full-width; action bar stacks vertically with full-width send button.
  - **CSS:** New `.gmail-compose`, `.gmail-toolbar`, `.gmail-fields`, `.gmail-field-row`, `.gmail-to-chip`, `.gmail-body-area`, `.gmail-body-text`, `.gmail-attachment-bar`, `.gmail-attachment`, `.gmail-action-bar`, `.gmail-send-btn`, `.gmail-copy-btn`, `.gmail-copied`, `.gmail-banner*`, `.gmail-footer-link`, `.gmail-go-to-call-btn` styles in `base.html`. Old `.drawer-email-*` / `.drawer-copy-btn` / `.drawer-mailto-btn` styles removed. Legacy `.drawer-btn` / `.drawer-btn-primary` / `.drawer-btn-secondary` retained for no-answer partial.
  - **JS:** `initDrawerEmailActions()` in `index.html` updated to target `.gmail-compose` container and `.gmail-copy-btn` / `.gmail-mailto-btn` elements. `htmx:afterSwap` handler updated to detect `.gmail-compose` instead of `.drawer-email-content`.
...
- **Advocacy page overhaul — outreach-first redesign (2026-02-17):**
  - **Hero/landing:** Hero restyled to match the rest of the advocacy page (2026-02-17): removed dark gradient and full-bleed; now uses same light background (#fff), Georgia/serif, #222/#444 text, bordered form (#fafafa + 1px solid #ccc), h1-style headline (border-bottom 2px solid #333), dark CTA button (#333). Trust line and labels use muted grays. Sits inside container with a bottom border instead of a separate marketing band. **Hero image centering (2026-02-17):** `.hero-image` in `base.html` now uses `display: block` and `margin-left/right: auto` so the Kei truck image is explicitly centered in the hero column.
  - **Auto-submit ZIP:** Typing the 5th digit auto-submits the form and smooth-scrolls to results. No button click needed.
  - **Outreach action buttons:** Each member card now has prominent Call / Email buttons (ILGA link in "Why we picked them" section):
    - **Call** opens a slide-in panel from the right with a phone script (the member's `script_hint`).
    - **Email** opens a sidebar with subject, body, placeholders, and "Open in email client" when member has email.
    - **Post on X removed (2026-02-17):** Dropped Twitter/X button and panel; we don't have member X handles and state-level evidence favors email/call. Outreach is Call + Email only.
  - **ILGA profile link (2026-02-17):** Removed ILGA as an outreach button; added a blue underlined "View on ILGA website" link at the bottom of the "About" (expandable) section (routes to member page on ILGA). Styles: `.ilga-profile-link` in `base.html`; `.outreach-profile` removed.
  - **Phone panel:** Slide-over panel (400px from right edge) with member name, clickable phone number, and the full call script. Closes with X or Done button. Overlay dims the page.
  - **Streamlined "Why this person matters":** Removed verbose power-context box. Replaced with compact signal chips (colored pills): committee chair, ranking, passage rate, persuadable. Only meaningful signals shown — no filler.
  - **Quick snaps (positive-only):** Chips now show only stats that make the member look like a strong contact. Committee chair shows advance rate only when ≥25%; otherwise "— gatekeeper for hearings". Added cross-party chip (≥15% bridge), "X laws passed" when productive (≥3 laws) but passage % &lt;15%, and "Top of Chamber" for top 30% rank. Low rates (e.g. 1% advance) are never shown.
  - **"Wow" quick snaps:** Lead with influence-network stats so callers think "they're great, I'll call them": (1) **Bills they co-sponsor pass at X× the chamber rate** (cosponsor_passage_multiplier ≥1.2), (2) **Outperforms caucus: X× average passage** (passage_rate_vs_caucus ≥1.2), (3) **High/moderate bipartisan reach**, (4) **Works with 40+ legislators**. These appear first; chair/rank/passage/bridge/persuadable follow. New `.signal-wow` style (amber) for the first two and network size.
  - **HTMX Advocacy drawer — Call → Follow-up Email (2026-02-17):**
    - **Single slide-out drawer** replaces separate Call and Email panels. Clicking **Call** opens drawer in CALL view; clicking **Email** opens in EMAIL view. Drawer prefilled for that legislator + user ZIP.
    - **Wrap Up (Option A) — 2026-02-17:** **CALL view** now uses an iOS/iMessage-style **Wrap Up** card instead of the old "After the call" form. **Conditional reveal (2026-02-17):** "Done! Tap the button…" banner removed; replaced with a red full-width **End call** button. On press, the Wrap Up panel fades in and moves 10px up (CSS transition). Card has 2 Settings-style rows (Staffer name, Email address) plus **Note** (textarea for extra notes). Single CTA: **"Draft follow-up email"**. HTMX POSTs to `/advocacy/call/<call_id>/wrapup` and swaps drawer body to **EMAIL view** (no intermediate confirmation). Email view: prefilled after-call subject/body; **To:** row (populated or "No email captured"); if no email → copy-only banner + Copy buttons + "Open in email client" (mailto with empty To). Bubble captures (email/name in script) sync into wrap-up form; known legislator email pre-fills wrap-up row. CSS: `.drawer-wrapup-card`, `.drawer-wrapup-row`, `.drawer-wrapup-cta` (iOS rounded card, row highlight on focus, iMessage-like primary button). Backend: `POST /advocacy/call/{call_id}/wrapup` (staffer_name, email_address, next_step, zip) returns email partial; stateless (no session store). Legacy `POST /advocacy/drawer/after-call` retained for compatibility.
    - **CALL view:** Kei/mini truck script + Wrap Up card (see above). **"Draft follow-up email"** POSTs to `/advocacy/call/<member_id>/wrapup`; drawer swaps to EMAIL view.
    - **After-call EMAIL view:** If email captured → **After-call follow-up email** (prefilled subject/body, To row, Copy + Open in email client). If no email → **To:** "No email captured", copy-only banner ("Copy the email below and ask for an email next call"), same template + Copy + Open in email client (mailto without To) + "Go to Call tab" when applicable.
    - **Email-first EMAIL view (from card):** If legislator has public email → recipient + **Email-first** template. If no public email → "Call to get best email" notice + template without recipient + **Go to Call tab**.
    - **Email templates (spec):** (A) After-call: subject "Follow-up to my call today — Kei/Mini Truck registration fix (ZIP …)", body thanks staffer, single ask, SOS materials. (B) Email-first: subject "Constituent request (ZIP …) — sponsor a kei/mini truck registration fix", body Dear [Legislator], request + "brief call with staffer" CTA.
    - **EMAIL view actions:** Copy Subject, Copy Body, Copy All, Open in email client (mailto with subject/body and recipient if available). Letter template PDF link.
    - **Backend:** `GET /advocacy/drawer?view=call|email&member_id=&zip=` returns partial (`_advocacy_drawer_call.html` or `_advocacy_drawer_email.html`). `POST /advocacy/drawer/after-call` (member_id, zip, staffer_name, got_email, email_address, instructions) returns email partial. Helpers: `_build_after_call_email_subject/body`, `_build_email_first_subject/body`, `_find_member_by_id()`. Card buttons pass `data-member-id` and `data-zip`; `openAdvocacyDrawer(view, id, zip)` sets drawer attributes, fetches partial, opens drawer; HTMX afterSwap updates title to "Follow-up email" when body is email content and binds copy/mailto buttons.
    - **Call script header layout (2026-02-17):** Call drawer header saves vertical space: profile **photo and name block (SENATOR + name) are on one row, left-aligned**; phone number pill stays on its own row **center-aligned**. Template: `_advocacy_drawer_call.html` uses `.drawer-call-profile-row` (flex) and `.drawer-call-name-block`; CSS in `base.html` (`.drawer-call-profile-row`, `.drawer-call-name-block`, `.drawer-call-number { text-align: center }`).
    - **Phone pill CTA animation (2026-02-17):** Green phone number pill in Call drawer has a subtle **ring** (scale 1 → 1.04 → 1 → 1.03 → 1 on a 2s loop) and **pulse** (expanding green box-shadow ring that fades) to draw attention without extra copy. Keyframes `phone-pill-ring` and `phone-pill-pulse` in `base.html`; applied to `.drawer-call-number-pill`; pulse speeds up slightly on hover. **Stop on end call (2026-02-17):** When the user ends the call (clicks "Yes, I reached someone" and Wrap Up panel is revealed), the pill animation stops via `.drawer-call-number-pill-ended` (animation: none, box-shadow: none); `initDrawerEndCall` adds that class to the pill when the wrap-up panel is shown.
    - **One-at-a-time pulse (2026-02-17):** Blue border pulse animates only one capture field at a time. Email input pulses first; after the user enters an email, the pulse moves to the name input. When the user enters a name, the name pulse stops. Implemented via `.drawer-capture-pulse` class (animation applied only when this class is present); `initDrawerCapturePulse(container)` in `index.html` toggles the class based on email/name values.
    - **Saved UI (2026-02-17):** After the user types in a field, the input is replaced by a clearer “saved” state. Email becomes a green **email saved** pill (checkmark + email, gradient background). Name becomes a **sticky note** (yellow/cream, slight rotation, tape strip, Caveat/cursive font). Inputs stay in DOM (hidden) so form copy on submit still works; `initDrawerCapturePulse` toggles `.drawer-email-saved-visible` / `.drawer-name-saved-visible` and syncs displayed text.
    - **Drawer capture UX (2026-02-17):** Email/name blocks right-aligned; email pill blue; both pill and sticky click-to-edit; wrap-up form auto-filled from capture (staffer_name, email) and known legislator email when no capture. syncCaptureToWrapupForm in initDrawerCapturePulse; member_public_email from backend; htmx:beforeRequest syncs bubble values into wrap-up form on submit.
    - **End Call button + smart Wrap Up (2026-02-18):** Replaced "Did you reach anyone?" outcome step with a single **full-width red "End Call" button** (phone icon rotated 135deg, #ff3b30) at the bottom of both call script and voicemail script. Since we know which script the user is on, no need to ask. On click, the **Wrap Up panel** slides in. **Conditional CTAs in Wrap Up:** If an email is available (captured in call, pre-filled from known email, or typed in wrap-up form) → **"Draft follow-up email"** (existing HTMX flow). If **no email** → "Draft follow-up email" is hidden; two alternatives shown: **"Set a reminder to follow up"** (green, opens Google Calendar with event 2 days out) and **"Call another member"** (secondary, closes drawer and scrolls to results so user can pick another card). **Voicemail mode:** End Call pre-fills note field with "Left voicemail". JS: `initDrawerEndCall` detects mode via `#drawer-end-call` (call) vs `#drawer-end-call-vm` (voicemail); `hasEmail()` checks captured input, wrap-up email field, and `data-known-email`; `showCtas()` toggles visibility. CSS: `.drawer-end-call-btn` (red, flex centered, phone icon), `.drawer-wrapup-ctas`, `.drawer-wrapup-cta-reminder` (green), `.drawer-wrapup-cta-secondary` (gray bordered) in `base.html`.
    - **No-answer hint above script (2026-02-18):** Between the gray header ("Read one bubble at a time…") and the "Introduce yourself" divider, a minimal **no-answer hint** strip: "No answer?" with a text-style button that **toggles in place** between call script and voicemail script (no HTMX swap). Styled to be visible but low-key: light gray background (#fafafa), muted text (#8e8e93), green underlined link-style button; `.drawer-no-answer-hint`, `.drawer-no-answer-hint-btn` in `base.html`. Template: `_advocacy_drawer_call.html`.
    - **Call script content (2026-02-18):** See **Outreach scripts — dynamic templates** above for current answered/voicemail/email copy. Backend: `GET /advocacy/drawer?view=call&member_id=&zip=&target_type=`; passes `office_name`, `district_label`, `legislator_last`, `target_type`. Card Call button has `data-target-type`; `openAdvocacyDrawer(view, id, zip, photoUrl, targetType)` sends target_type for call view.
    - **Constituent checkbox (2026-02-18):** Call drawer has an **"I'm a constituent"** checkbox between the no-answer hint and the "Introduce yourself" divider. When checked (default when user searched by ZIP), the intro and voicemail bubbles are prefilled with "I'm a constituent in ZIP XXXXX" using the drawer's search ZIP. When unchecked, copy shows "I'm calling from [CALLER_CITY] [CALLER_ZIP]". JS: `initDrawerConstituentCheckbox(container)`; CSS: `.drawer-constituent-hint`, `.drawer-constituent-checkbox-wrap`, `.drawer-constituent-desc` in `base.html`.
    - **Voicemail script toggle (2026-02-18):** Clicking "Switch to voicemail script" hides the talking bubbles and shows a **concise voicemail script** — single bubble with short script (intro, 3-401(c-1), dynamic ASK block, call-back number). Copy emphasizes follow-up. **Remind me to follow up:** "Add reminder to follow up" opens reminder modal (date/time, Google Calendar, email myself, copy link). Both views end with the same red **End Call** button → Wrap Up panel with conditional CTAs. JS: `initDrawerVoicemailToggle(container)` in `index.html`; CSS: `.drawer-voicemail`, `.drawer-voicemail-intro`, `.drawer-voicemail-single`, `.drawer-voicemail-reminder`, `.drawer-reminder-link` in `base.html`.
    - **Reminder modal (2026-02-18):** "Add reminder to follow up" (voicemail bubble) and "Set a reminder to follow up" (Wrap Up panel) now open a **reminder modal** instead of going straight to Google Calendar/mailto. Modal has **date** and **time** inputs (default: 2 days from now, 9:00 AM), then: **Add to Google Calendar** (opens Calendar with that date/time and event **details** containing the exact **call script URL** so the user can jump back), **Email myself a reminder** (mailto with subject/body and the same link in the body), **Copy link only** (for pasting into native phone/desktop reminders). No native Reminders API from the web; Google Calendar is the primary path; copy link supports manual paste into iOS Reminders, etc. (Deep link auto-open from URL params was removed 2026-02-18; reminder links still contain the call script URL for manual use.) JS: `buildCallScriptUrl(memberId, zip)`, `openReminderModal()`, `closeReminderModal()`, `initReminderModal()`; modal HTML in `index.html`, CSS `.reminder-modal*` in `base.html`.
  - **Call / Email panels (legacy, 2026-02-17):** Replaced by single Advocacy drawer above. Script sections and email copy were previously in separate panels; now unified in one drawer with CALL and EMAIL views.
  - **Letter template:** `GET /advocacy/letter-template` serves `letter_template.html` — generic constituent letter with placeholders; instructions to print/save as PDF and attach to email.
  - **"Why we picked them" (Option C, 2026-02-17):** Renamed expandable section from "More details" to "Why we picked them". Added a 2–3 sentence human-readable summary at the top of the expanded section (built from rank, passage vs caucus, co-sponsor lift, bipartisan, chair, persuadable, bridge). Full Legislative Scorecard and rest of details unchanged. CSS: `.why-picked-prose` in `base.html`.
  - **Why we recommend them — name usage (2026-02-17):** Replaced "they/their/them" in the paragraph under the member name with: **first sentence** = full name (e.g. "Maria Martinez ranks #1…"); **second sentence** = first name (e.g. "Maria's own sponsored bills… so Maria has a strong track record"); **third sentence** = title + last name (e.g. "Rep. Martinez" or "Sen. Martinez"). Verb agreement updated (has/tends/works/co-sponsors/chairs) when using names. Template: `_results_partial.html` (name parts, `_title_last`, per-sentence `_ref_subj`/`_ref_poss`).
  - **Naming pass across templates (2026-02-17):** Ensured member-specific copy uses names instead of "they/their/them" everywhere: (1) **`_results_partial.html`:** Role tooltip for rep now "Your state representative votes on bills…". Signal chips and their tooltips use `_first_name` (e.g. "Bills {{ _first_name }} co-sponsors pass at…", "{{ _first_name }}'s own sponsored bills…", "Share of {{ _first_name }}'s primary-sponsored bills…", persuadable "{{ _first_name }} has voted against the party line…"). Scorecard Moneyball note "how far {{ _first_name }}'s bills go". Aside formula glossary "the member co-sponsors with" (generic). (2) **`intelligence_member.html`:** Section heading "Where {{ member.name }}'s Influence Comes From"; Sponsor Pull line "Bills {{ _first_name }} sponsors are…". (3) **`index.html`:** Email notice when no public email: "Call the office" / "paste the address" (no "their").
  - **Why we picked them — pizzazz + top 3 stats (2026-02-17):** Prose now uses **bold** for key stats and phrases (e.g. **#1 in House**, **2.8× caucus average**, **pick winners**, **constituent pressure can matter**) and includes numbers wherever possible (rank, multiplier, defection count, %). Yellow compact scorecard now shows the **3 most important stats** per member (priority: chair → chamber rank → passage vs caucus → Moneyball → laws passed → cross-party → persuadable → co-sponsor lift → bipartisan → network → passage rate → active bills). Fallback for minimal data: district role line or laws/Moneyball if present.
  - **Collapsed details:** Scorecard, voting record, committee assignments, and moneyball breakdown are all behind a single expandable `<details>` toggle. Keeps the card focused on outreach.
  - **Bio vs "Why we recommend them" swap (2026-02-17):** Swapped positions: (1) **In the card header** (where the short bio was): now shows **"Why we recommend them"** — the 2–3 sentence prose + compact scorecard (same content and styles as before). (2) **In the expandable section** (where "Why we picked them" was): now **"About"** — full bio (`bio_text`) with no truncation (no "Read more" / line-clamp). Committees, voting record, and full scorecard remain inside the same "About" dropdown. Renamed the expandable section from "Why we picked them" to "About"; the inline block is labeled conceptually as "Why we recommend them" (no truncation of about text since it's in the dropdown). Template: `_results_partial.html` — vars moved to top of card macro; header has why_picked prose + compact-scorecard; details summary is "About", body leads with full `.card-bio`, then existing detail sections.
  - **Quick bio (legacy note):** Bio was previously in header with truncation; now lives in "About" dropdown as full text. `.card-bio` / `.card-bio-text` styles still used for the About section; no "Read more" in dropdown.
  - **Stat blurb tooltips — copyable + numbers (2026-02-17):** All stat blurb tooltips (bipartisan reach, large network, chair, passage, etc.) are now click-to-open: add `tooltip-click` and chip `onclick` toggle so the tooltip stays open and text can be selected/copied. Click outside (existing handler) closes. Each tooltip includes a **Stats:** line with actual numbers: co-sponsor passage (rate vs chamber median and ×); caucus (× and caucus avg %); bipartisan (% from other party, D/R counts, unique collaborators); large network (unique count); chair (advancement % and bill count when present); ranking (# of N); passage (X of Y bills, %); laws passed (X of Y); cross-party (% and bills filed); persuadable (defection count). CSS: `.tooltip-wrap.tooltip-click .signal-chip { cursor: pointer; }` in `base.html`.
  - **Call/Email buttons restyle (2026-02-17):** Same row, half-width each; solid fill with white icon + text: green (#1a6b3c) for Call, blue (#2a5aad) for Email; no border; hover brightness; responsive keeps side-by-side. CSS: `.outreach-row` (flex nowrap), `.outreach-btn` (flex 1 1 0, color #fff, border none), `.outreach-phone`/`.outreach-email` (background only) in `base.html`.
  - **Outreach counts + heat score + mock DB (2026-02-17):** (1) Call/Email counts under each button (hide when 0). (2) Heat score pill top-right of card (minimal style); 🔥 + unique outreach; only when > 0. (3) Mock DB: `mocks/dev/outreach.json` (and `cache/outreach.json` if present) keyed by member_id with `calls`, `emails`, `unique_outreach`. Loaded at startup; `_member_to_card()` adds `outreach_calls_count`, `outreach_emails_count`, `heat_score`. AppState.outreach; `_outreach_val(member_id, key)`. Template: heat pill, `.outreach-cell`, `.outreach-count`. CSS: `.heat-score-pill`, `.outreach-cell`, `.outreach-count`.
  - **CSS:** New styles for `.advocacy-hero`, `.outreach-row`/`.outreach-btn`, `.signal-chip`, `.phone-panel`, `.card-details`. All in `base.html`.
  - **Results layout alignment (2026-02-17):** First h2 ("Your Legislators") in `.results-main` has no top margin (`.results-main > h2:first-of-type { margin-top: 0 }`); `.results-main` has `padding-top: 8px` so the main column aligns with the "How we pick targets" aside title.
  - **Aside bar: Super Ally + chips note (2026-02-17):** "How we pick targets" aside now includes (1) **Super Ally** — when Power Broker and Potential Ally are the same person (both top influence and seatmate; uniquely high-value target). (2) **Note** — "The chips on each card show *why* we recommend them, not how we picked the category." Styled with `.hiw-note` (muted, bordered above) in `base.html`.
  - **Chips section label (2026-02-17):** Each card’s signal chips now have a small title above them: **"Why we recommend them"** (`.chips-label` in `_results_partial.html`), so it’s clear the chips are evidence for the recommendation, not the selection category. Style: 0.85em, semibold, #555, in `base.html`.
  - **Chips that flow + mobile abbreviations (2026-02-17):** Signal chips now flow as compact pills (flex-wrap, align-items: flex-start, flex: 0 1 auto). On viewports ≤768px, abbreviated chip text is shown so pills stay short and wrap naturally: e.g. "Cosponsor pass 1.5×", "1.2× caucus", "Bipartisan", "40+ colleagues", "Chair: Committee Name", "#1 in Senate", "15% passage", "15% cross-party", "Persuadable (5×)". Full text remains in tooltips. Template: `_results_partial.html` uses `.chip-text-full` / `.chip-text-mobile`; CSS in `base.html` toggles visibility by breakpoint.
  - **Chips sorted by ML priority (2026-02-17):** "Why we recommend them" chips are now ordered by ML/influence priority for all members. Backend: `RECOMMENDATION_CHIP_PRIORITY` and `_recommendation_chip_order()` in `main.py`; each card gets `recommendation_chip_order` (list of active chip keys in priority order). Template: `_results_partial.html` iterates over `member.recommendation_chip_order` and renders each chip by key. Priority order matches the influence formula: co-sponsor lift (sponsor pull) and outperforms caucus first, then bipartisan reach, big network, chair, high rank, passage/productive, cross-party, persuadable.
  - **Moneyball definition in aside (2026-02-17):** The "Moneyball" underlined term with "?" in "How we pick targets" now expands a definition **directly below** in the same aside (no floating tooltip). Definition text is in italics; formula is textbook-style (serif, block layout). Toggle: click "Moneyball" or "?" to show/hide; click outside to close. Keyboard: Enter/Space on trigger toggles. Template: `_results_partial.html` (`.moneyball-aside-trigger` + `.moneyball-definition-block`); CSS in `base.html` (`.moneyball-definition-block`, `.moneyball-formula-textbook`); toggle + click-outside in `index.html`.
  - **Overall scorecard transparency (2026-02-17):** In the Legislative Scorecard "Overall" section, "Total bills" and "Total passed" now show the full composition in parentheses so it’s explicit what’s included: e.g. "Total bills (20 laws + 4 resolutions)" and "Total passed (0 laws + 3 resolutions)". Uses `laws_filed`/`laws_passed` and `resolutions_filed`/`resolutions_passed` from the scorecard; `.scorecard-note` styling unchanged.
  - **Moneyball in full scorecard (2026-02-17):** Full Legislative Scorecard ("Why we picked them" → Full Legislative Scorecard) now includes a **Moneyball** section when the member has Moneyball data: Score, Effectiveness %, Pipeline depth (avg), Magnet, Bridge %, Network centrality, Institutional weight. Rendered after Lawmaking and Overall; uses existing `member.moneyball` and `member.moneyball_score` from `_member_to_card()`.
  - **Moneyball explanation and score in header (2026-02-17):** (1) **Score next to section title:** Full Scorecard Moneyball section header now shows "Moneyball (36.4)" so the overall score is visible beside the title; formula note row added: "Weighted composite: 24% effectiveness, 16% pipeline, 16% magnet, 12% bridge, 12% network, 20% institutional." (2) **Tooltip next to name:** Clicking "?" next to Moneyball in the compact scorecard now shows component weights and short definitions (Passage 24%, Pipeline 16%, Co-sponsors/Magnet 16%, Cross-party/Bridge 12%, Network 12%, Institutional 20%). CSS: `.scorecard-formula-note`, `.tooltip-weights-title`, `.tooltip-weights-list` in `base.html`. (3) **Inline definitions in table:** Each Moneyball row now has a brief inline explanation (e.g. "Effectiveness — laws passed ÷ laws filed (HB/SB)", "Magnet — avg co-sponsors per law") using `.scorecard-note`.

- **Member photos + partial scrape (2026-02-17):**
  - **photo_url:** Member model and scraper now capture the ILGA member photo from each detail page (`img.member-photo` → `src`). Stored in cache as `photo_url`; exposed on GraphQL `MemberType`, advocacy card dict, and templates.
  - **make scrape** — Full scrape now includes photo for each member.
  - **make scrape-members** (or `scripts/scrape.py --members-only`) — Fetches only members + committees; loads bills from cache. Use after deleting `members.json` to repopulate members (with photos) without re-scraping bills or votes.
  - **make refresh-photos** (or `scripts/refresh_member_photos.py`) — When you have existing `cache/members.json` and `cache/bills.json`, re-fetches each member’s detail page and updates only `photo_url`, then saves. No bill or vote scrape.
  - **Call script drawer — member photo in circle (2026-02-17):** The Call script drawer header now shows the member's photo in the circular placeholder when available. Fix: (1) Drawer stores `data-drawer-photo-url` when opened. (2) `openAdvocacyDrawer` sends `photo_url` in the fetch (from the button's `data-photo-url` or the drawer's stored value). (3) "Go to Call" in the email drawer now passes the stored photo URL so switching to Call view shows the photo. Backend already used query param or member's `photo_url` and normalizes relative URLs to absolute ILGA URLs. (4) **Client-side fallback:** After loading drawer HTML, JS sets the header img src from `data-drawer-photo-url` (replacing placeholder or updating img) so the photo shows even if the server response didn't include it. Drawer response sends `Cache-Control: no-store` to avoid stale cached HTML.

- **Responsive layout + scripts sidebar (2026-02-17):**
  - **Site responsive:** Breakpoints in `base.html`: 768px (tablet) and 480px (phone). Body/container padding reduced on small screens; hero form stacks (column); hero trust line wraps per item on phone; hero image scales.
  - **Results sidebar ("How we pick targets"):** On ≤768px, `.results-layout` stacks: `.results-aside` moves above `.results-main`, full width, no sticky. Sidebar block is now a collapsible `<details>` in `_results_partial.html` so users can collapse/expand it on any screen; summary styled like original title with triangle indicator.
  - **Script panels (Call/Email):** At 768px and below, slide-over panels use full width (100%); footer buttons wrap; on phone footer buttons stack (column) with full-width primary actions.
  - **Cards & outreach:** Card header wraps; member photo scales down on tablet/phone; outreach row stacks on phone; tooltips constrained to viewport.
  - **Horizontal overflow fix (2026-02-17):** Prevented content from being cut off on narrow viewports: `html` and `body` use `overflow-x: hidden`; `.container` has `width: 100%` so it never exceeds viewport; `.card`, `#results`, `.results-layout`, `.results-main` use `min-width: 0` and/or `overflow-wrap: break-word` so flex children can shrink and text wraps; `.signal-chip` and `.power-badge` no longer use `white-space: nowrap` and instead use `overflow-wrap: break-word` so long labels (e.g. "Vice-Chairperson: Health Care Licenses") wrap inside pills; `.why-picked-prose`, `.compact-scorecard`, `.section-desc`, `.results-main h2` all wrap with `word-break: break-word` / `overflow-wrap: break-word`.
...
- **ML pipeline diagnostics (2026-02-13):**
  ...
  - **Schema fix (2026-02-18):** `process_bills()` in `ml/pipeline.py` — normalized types so Polars does not infer mixed columns: `primary_sponsor_id` coerced to `str | None`, `bill_number_raw` always `str`, and `_parse_bill_number()` accepts non-string raw. Fixes `ComputeError: could not append value "3268" of type: str to the builder` when cache has mixed int/str sponsor IDs.
- **Full member roster / missing data (2026-02-18):** If scrape reports "20 members" and bills.json has sparse data (e.g. empty sponsor_ids), the scraper is **reusing** `cache/members.json` instead of re-fetching from ILGA. Members are loaded from cache when present; only bills are updated incrementally. **Fix:** `make scrape-full-members` (removes `cache/members.json` then runs scrape-members) to re-fetch the full ~177 members from ILGA while keeping existing bills. For a full reset: `make scrape FRESH=1`. Documented in `docs/reference/cli-make.md`.
- **Single command for full scrape (2026-02-18):** Added **`make scrape-full`** = `make scrape FRESH=1 FULL=1`. Use when the data pipeline is "wonky" (e.g. only 20 or 60 members, missing House): wipes `cache/`, then scrapes all members (no limit) + full bill index + ML. Documented in Makefile, `docs/getting-started.md`, and `docs/reference/cli-make.md`.
