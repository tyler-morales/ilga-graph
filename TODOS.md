...
- **main.py split into focused modules (2026-02-18):**
  ...
  - **member_lookup.py** — `find_member_by_id(state, id)`, `find_member_by_district(state, chamber, district)` used by explore graph API (advocacy uses `advocacy_helpers`).
  - **routers/** — Package added for future APIRouter modules (advocacy, intelligence, explore); not yet wired.
...
- **Advocacy helpers extracted from main.py (2026-02-18):**
  - Moved all advocacy-specific logic into `src/ilga_graph/advocacy_helpers.py`: card building (`member_to_card`), recommendation chip order, influence dict, script/email builders, `find_power_broker`, `find_ally`, `find_member_by_id` / `find_member_by_district`, `committee_member_ids`, `test_member_list`. Each helper that needs app state takes `state` as first argument; main.py route handlers call `advocacy_helpers.*(state, ...)`. Reduces main.py size and keeps advocacy logic in one place.
- **Backlog:**
...
- **Gmail-style email drawer redesign (2026-02-18):**
  ...
  - **"I sent" + simplified copy (2026-02-18):** Next to "Open in email client" added **"I sent"** button (id `gmail-sent-email-btn`) so advocates can record that they actually sent the email; when signed in this POSTs to `/outreach/record` (kind=email), shows "Recorded!" then "I sent", and the card updates to gold "Emailed!" badge. Copy Subject/Body/All pills removed from the action bar to save space; replaced with a single **"Copy subject & body"** text link in a thin row above the action bar (`.gmail-copy-row`, `.gmail-copy-link`). JS: `initDrawerEmailActions` uses restore label from button text so the copy link reverts to "Copy subject & body" after "Copied!"; "I sent" success feedback only when `data.ok` (signed-in). CSS: `.gmail-sent-btn` (outline secondary), `.gmail-copy-row`, `.gmail-copy-link` in `base.html`.
