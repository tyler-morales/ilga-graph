...
**Sitemap and robots:** The app serves `/sitemap.xml` (key pages: `/`, `/advocacy`, `/intelligence`, `/intelligence/money`, `/money`, `/money/demo`, `/money/signup`, `/explore`) and `/robots.txt` (allow all, with a `Sitemap:` line). Both use `ILGA_APP_BASE_URL` for absolute URLs, so set it in production for correct discovery by search engines.
