...
| **Master timeline** | The phased plan from now to bill signed, shown on `/timeline` as a waterfall (Gantt-style) with a time axis and phase bars. Source: `TIMELINE_PHASES`. The table has three row types: **Phase** (name + date range + bar), **Phase description** (what we’re doing in that phase; text flows across the full row), and **Phase items** (specific milestones with dates and descriptions). Each phase has a date range and optional milestones. |
...
| **Drawer** | The slide-out panel for call scripts and email templates. Opens from “Reach out” on a legislator card. Step-by-step flow definitions (call/email slugs) are in Admin → **Flow definitions** (`/admin/flows`). |
| **Funnel** | The user journey from page visit to completed outreach. Measured for conversion (e.g. % who opened drawer and completed at least one call/email). Step slugs and why-you-care branch content: Admin → **Flow definitions** (`/admin/flows`). |
