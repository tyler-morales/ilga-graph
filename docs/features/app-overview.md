...
- **Share cards and analytics** — The base template sets Open Graph and Twitter Card meta so shared links show a title, description, and image. Canonical URL and a default meta description are set from `ILGA_APP_BASE_URL` and config. Optional [Umami](https://umami.is) analytics: set `ILGA_UMAMI_WEBSITE_ID` (e.g. in prod) to inject the tracking script; no cookies, 100k events/month on the free tier.
