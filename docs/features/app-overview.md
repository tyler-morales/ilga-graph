...
- **Loading facts** — When the ZIP search loading animation is enabled (feature flag), the "finding targets" state cycles through short kei truck facts (e.g. Texas 2025, 25+ year rule, fun trivia) and data-driven stats (e.g. legislator count, ZIPs mapped) for light gameification while results load.
