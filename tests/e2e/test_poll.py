...
def test_home_poll_renders(page, base_url):
    ...
    assert page.locator("#why-you-care-flow form, #why-you-care-flow .wyc-branch-content").first.is_visible()
...
def test_home_poll_submit_anon(page, base_url):
    ...
    submit = page.locator("#home-kei-poll-form").get_by_role("button", name="Submit and see results")
    ...
    page.locator("#why-you-care-flow .kei-status-success, #why-you-care-flow .wyc-branch-content").first.wait_for(
        state="visible", timeout=10000
    )
    assert page.locator("#why-you-care-flow").locator(".kei-status-success, .wyc-branch-content").first.is_visible()
...
def test_standalone_poll_submit(page, base_url):
    ...
    submit = page.locator("#standalone-kei-poll-form").get_by_role("button", name="Submit and see results")
    submit.wait_for(state="visible", timeout=3000)
    page.wait_for_timeout(300)
    submit.click()
    # HTMX swaps innerHTML of #standalone-kei-poll-wrap with _kei_poll_anonymous_success (no full redirect).
    page.locator("#standalone-kei-poll-results-drawer, .kei-poll-drawer, .poll-standalone__results").first.wait_for(
        state="visible", timeout=10000
    )
    assert (
